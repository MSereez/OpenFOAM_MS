...
void Foam::functionObjects::forces::createFiles()
{
	...
	bodyforceFilePtr_ = createFile("bodyforce");
        writeIntegratedBodyFMHeader("bodyForce", bodyforceFilePtr_());
        bodymomentFilePtr_ = createFile("bodymoment");
        writeIntegratedBodyFMHeader("bodyMoment", bodymomentFilePtr_());
...
void Foam::functionObjects::forces::writeIntegratedBodyFMHeader
(
    const word& header,
    Ostream& os
) const
{
    writeHeader(os, header);
    writeHeaderValue(os, "Body sys origin", bodySys_.origin());
    writeHeader(os, "");
    writeCommented(os, "Time");
    writeTabbed(os, "(total_x total_y total_z)");
    writeTabbed(os, "(pressure_x pressure_y pressure_z)");
...
)
{
    ...
    else if (dict.readIfPresent<point>("BodyAxisOrigin", bodySys_.origin()))
    {
bodyAxisSystem_ = true;
	if(bodyAxisSystem_)
	{	
	Info << "USING BODY AXIS SYSTEM " << endl;
	vector vectorProd = (beginBodyAxisX_ ^ beginBodyAxisY_);
	//Info << " scalarProd scalarProd " << vectorProd << endl;

	if(vectorProd!= beginBodyAxisZ_)
	{
		 FatalErrorInFunction
                << "Not a right handed LCS! " << vectorProd 
                << exit(FatalError);
...
void Foam::functionObjects::forces::writeIntegratedBodyForceMoment
(
    const string& descriptor,
    const vectorField& fm0,
    const vectorField& fm1,
...
void Foam::functionObjects::forces::writeBodyForces()
{
    Log << type() << " " << name() << " write:" << nl;
    
   // calcBodyForcesMoment();
    writeIntegratedBodyForceMoment
    (
        "Body forces Fx,Fy,Fz",
        bodySys_.localVector(force_[0]),
        bodySys_.localVector(force_[1]),
        bodySys_.localVector(force_[2]),
        bodyforceFilePtr_
    );
    ...
    (
        ...
        bodySys_.localVector(moment_[0]),
        bodySys_.localVector(moment_[1]),
        bodySys_.localVector(moment_[2]),
        bodymomentFilePtr_
    );
...
:
    ...
    pRef_(0),
    t_(0.0),
    beginBodyAxisX_(Zero),
    beginBodyAxisY_(Zero),
    beginBodyAxisZ_(Zero),
    quaternBX_(Zero),
    quaternBY_(Zero),
    quaternBZ_(Zero),
    rotationAxis_(Zero), 
...
dict.lookup("beginBodyAxisX") >> beginBodyAxisX_;
	dict.lookup("beginBodyAxisY") >> beginBodyAxisY_;
	dict.lookup("beginBodyAxisZ") >> beginBodyAxisZ_;
	dict.lookup("rotationAxis") >> rotationAxis_;
        dict.lookup("rotationOmega") >> rotationOmega_;
    ...
    if (porosity_)
    {
        ...
        forAllConstIters(models, iter)
        {
            ...
            for (const label zonei : cellZoneIDs)
            {
                ...
                const vectorField Md(d - bodySys_.origin());
...
void Foam::functionObjects::forces::getQuaternionBodyAxis()
{
        t_ = mesh_.time().value();
        scalar phi2 = rotationOmega_*t_;

	
        quaternion R11(rotationAxis_, phi2);
	tensor rotationTensor2 = R11.R();
  	// axesRotation  axes0_(rotationTensor);
	//Info << " Rotation Tensor " << rotationTensor << endl;

         quaternBX_ = rotationTensor2 & beginBodyAxisX_;
	 quaternBY_= rotationTensor2 & beginBodyAxisY_;
	 quaternBZ_= rotationTensor2 & beginBodyAxisZ_;


}

void Foam::functionObjects::forces::calcBodyForcesMoment()
{
	    word bodyaxissystem; 
    point origin1 = bodySys_.origin();
	   bodySys_ = coordinateSystem(bodyaxissystem,origin1,quaternBZ_,quaternBX_);
        vectorField localForceN(bodySys_.localVector(force_[0]));
        vectorField localForceT(bodySys_.localVector(force_[1]));
        vectorField localForceP(bodySys_.localVector(force_[2]));
        vectorField localMomentN(bodySys_.localVector(moment_[0]));
        vectorField localMomentT(bodySys_.localVector(moment_[1]));
        vectorField localMomentP(bodySys_.localVector(moment_[2]));


	

	vectorField  localForce1(localForceN + localForceT + localForceP);
	vectorField  localMoment1(localMomentN + localMomentT + localMomentP);
	
        bodyForces_ = sum(localForce1);
        bodyMoment_ = sum(localMoment1);

	Info << "body Forces " << bodyForces_ << endl;
	Info << "body Moment " << bodyMoment_ << endl;
}
